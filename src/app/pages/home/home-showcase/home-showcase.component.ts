            ...
            for (const i in arrayProducts) {
              if (category.url === arrayProducts[i].category) {
                ...
                if (arrayProducts[i].offer !== '') {
                  offerDate = new Date(
                    parseInt(
                      JSON.parse(arrayProducts[i].offer)[2].split('-')[0], 10
                    ),
                    parseInt(
                      JSON.parse(arrayProducts[i].offer)[2].split('-')[1], 10
                    ) - 1,
                    parseInt(
                      JSON.parse(arrayProducts[i].offer)[2].split('-')[2], 10
                    )
                  );
                  ...
                  if (today < offerDate) {
                    ...
                    if (type === 'Fixed') {
                      offer = value;
                      value = Math.round(
                        (offer * 100) / arrayProducts[i].price
                      );
                    }
                ...
                for (
                  let f = 0;
                  f < JSON.parse(arrayProducts[i].reviews).length;
                  f++
                ) {
                  totalReview += Number(
                ...
                const rating = Math.round(
                  totalReview / JSON.parse(arrayProducts[i].reviews).length
                );
                ...
                const arrayRating = $('.productRating');

                for (let j = 0; j < arrayRating.length; j++) {
                  for (let f = 1; f <= 5; f++) {
                    $(arrayRating[j]).append(`<option value="2">${f}</option>`);

                    if (rating === f) {
                      $(arrayRating[j]).children('option').val(1);
                    }
                  }
                }
